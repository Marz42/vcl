...
**Summary:** B0–B13, B16, B17, **B14 (live PASS)**, and **B15 (Local Audit UI)** are on the living tree (Known P0: **0**; standalone `bash tests/test-fleet.sh` **496**). Remaining NOT READY gap: live **`0.3.0 → 0.3.1-dev` upgrade** (`docs/release-readiness-0.3.1.md`). Operator runbook: `docs/live-replace-checklist.md`. The 0.3.0 freeze record is read-only.

- **v0.32 / UI security:** Local Audit UI refuses `reseed`; `/api/*` requires loopback Host + `X-Vincula-UI-Token`; POST requires `application/json` and a matching Origin. GET audit is cache-only (no SSH). Audit pages 500 rows / 31-day window. Fleet helpers return payloads (no `sys.stdout` swap). CSP / frame-deny headers; `/api/meta` says `identity_mutations: false`, `reseed: cli-only`. P1-05/P1-06 remain ops (branch protection / live upgrade).
