  ...
  async function loadOperations() {
    try {
      ...
      $("#operations-table").innerHTML = rows
        ? table(["TIME", "OPERATION", "TARGET", "STATE", "RESULT"], rows)
...
source: ${escapeHtml(u.source || "—")}</pre>
  ...
  async function runProbe() {
    try {
      toast(`${SSH_HINT} Running live probe…`);
  ...
  function wire() {
    ...
    $("#btn-refresh-users").addEventListener("click", async () => {
      try {
        toast(`${SSH_HINT} Refreshing users…`);
