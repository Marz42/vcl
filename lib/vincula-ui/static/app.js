...
(function () {
  ...
  const STATE_CLASS = new Set(["OK", "STALE", "FAIL", "UNKNOWN", "WARN", "-", "DEGRADED", "EMPTY", "HEALTHY", "SUSPECT", "UNREACHABLE", "RECOVERING"]);
  ...
  async function loadNodes() {
    ...
    const target = $("#monitor-table");
    try {
      const monitoring = await api("/api/monitor");
      const metric = (v, format) => v == null ? "—" : format(v);
      const rows = (monitoring.nodes || []).map((n) => {
        const m = n.metrics || {};
        const services = n.services || {};
        const serviceState = (v) => v == null ? "unknown" : v ? "active" : "inactive";
        const dims = ["node", "observation", "proxy", "accounting"].map((key) => {
          const h = (n.health || {})[key] || {};
          return `<td title="${escapeHtml(h.reason || "NO_OBSERVATION")}">${statePill(h.state)}</td>`;
        }).join("");
        return `<tr><td>${escapeHtml(n.name)}</td>${dims}<td>${escapeHtml(formatCacheAge(n.telemetry_age_seconds))}</td>
          <td>${metric(m.load1, (v) => Number(v).toFixed(2))}</td><td>${metric(m.memory_used_bytes, human)} / ${metric(m.memory_total_bytes, human)}</td>
          <td>${metric(m.disk_used_bytes, human)} / ${metric(m.disk_total_bytes, human)}</td><td>${metric(m.connections, (v) => escapeHtml(v))}</td>
          <td>${metric(m.rx_bytes_per_second, human)}/s ↓ ${metric(m.tx_bytes_per_second, human)}/s ↑</td>
          <td>sing-box: ${serviceState(services.sing_box_active)}<br>accountd: ${serviceState(services.accountd_active)}</td>
          <td>${escapeHtml((n.probe || {}).reason || "NOT_CONFIGURED")}<br>${metric((n.probe || {}).ttfb_ms, (v) => Number(v).toFixed(0) + " ms TTFB")}</td></tr>`;
      }).join("");
      target.innerHTML = `<p class="muted">Cache: ${escapeHtml(monitoring.cache_state)}. Proxy UNKNOWN means no verified probe result.</p>
        <table><thead><tr><th>Node</th><th>Reachability</th><th>Observation</th><th>Proxy</th><th>Accounting</th><th>Age</th><th>Load</th><th>RAM</th><th>Disk</th><th>Connections</th><th>Network</th><th>Services</th><th>Proxy probe</th></tr></thead><tbody>${rows}</tbody></table>`;
    } catch (_) {
      target.textContent = "Monitoring cache unavailable.";
    }
  }
