...
assert_equal "vcl-fleet version" "vcl-fleet 0.3.1-rc1" \
  "$(python3 "${PROJECT_DIR}/bin/vcl-fleet" version)"
assert_equal "vcl-fleet.py version" "vcl-fleet 0.3.1-rc1" \
  "$(fleet version)"
...
assert_success "verify reports lax version" grep -q '0.3.1-rc1' <<< "$verify_out"
...
assert nodes["lax"]["vincula_version"] == "0.3.1-rc1"
