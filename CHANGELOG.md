...
**Summary:** B0–B13, B16, B17, **B14 (live PASS)**, and **B15 (Local Audit UI)** are on the living tree (Known P0: **0**; standalone `bash tests/test-fleet.sh` **499**). Remaining NOT READY gap: live **`0.3.0 → 0.3.1-dev` upgrade** (`docs/release-readiness-0.3.1.md`). Operator runbook: `docs/live-replace-checklist.md`. The 0.3.0 freeze record is read-only.

- **v0.32 follow-up:** `fleet_op_lock` takes a per-thread `threading.RLock` before the cross-process flock (concurrent UI `/api/sync` cannot skip the mutex). Audit destination substring is applied in SQL before `LIMIT`/cursor. Busy lock maps to HTTP 409. POST Origin is checked when present (token remains required).
