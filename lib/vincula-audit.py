#!/usr/bin/env python3
"""vincula-audit — connection-level RFC3339 interval queries.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Vincula connection audit (RFC3339 interval-overlap)"
    )
    p.add_argument("--db", required=True, help="path to accounting.db")
    p.add_argument("--users", required=True, help="path to users.json")
    p.add_argument("--from", dest="from_ts", required=True, metavar="RFC3339")
    p.add_argument("--to", dest="to_ts", required=True, metavar="RFC3339")
