...
assert_equal "vcl-fleet version" "vcl-fleet 0.2.9" \
  "$(python3 "${PROJECT_DIR}/bin/vcl-fleet" version)"
assert_equal "vcl-fleet.py version" "vcl-fleet 0.2.9" \
  "$(fleet version)"
...
assert_success "verify reports lax version" grep -q '0.2.9' <<< "$verify_out"
...
assert nodes["lax"]["vincula_version"] == "0.2.9"
