...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  finish_fleet() {
    trap - EXIT
    # §5: real HOME must be untouched when suite ran standalone or under test.sh.
    local real_home="${FLEET_SAVED_HOME:-${SAVED_SUITE_HOME:-}}"
    local real_kh_fp="${FLEET_REAL_KH_FP:-${SUITE_REAL_KH_FP:-}}"
    if [[ -n "${real_home}" ]]; then
      local real_kh="${real_home}/.ssh/known_hosts"
      if [[ -n "${real_kh_fp}" ]]; then
        if [[ -f "$real_kh" ]]; then
          local after_fp
          after_fp=$(sha256sum -- "$real_kh" 2>/dev/null | awk '{print $1}')
          if [[ "$after_fp" == "$real_kh_fp" ]]; then
            pass "P1 §5: real HOME known_hosts fingerprint unchanged (fleet)"
          else
            fail "P1 §5: real HOME known_hosts fingerprint changed (fleet)"
          fi
        else
          fail "P1 §5: real HOME known_hosts missing after fleet suite"
        fi
      elif [[ -f "$real_kh" ]]; then
        fail "P1 §5: fleet suite created known_hosts under real HOME"
      else
        pass "P1 §5: real HOME had no known_hosts (fleet; still absent)"
      fi
    fi
...
# When sourced from test.sh, HOME is already the suite temp home — capture the
# original user HOME for §5 fingerprint checks.
if [[ -n "${SAVED_SUITE_HOME:-}" ]]; then
  FLEET_SAVED_HOME="${SAVED_SUITE_HOME}"
fi
FLEET_REAL_KH_FP="${SUITE_REAL_KH_FP:-}"
if [[ -z "${FLEET_REAL_KH_FP}" && -n "${FLEET_SAVED_HOME}" && -f "${FLEET_SAVED_HOME}/.ssh/known_hosts" ]]; then
  FLEET_REAL_KH_FP=$(sha256sum -- "${FLEET_SAVED_HOME}/.ssh/known_hosts" 2>/dev/null | awk '{print $1}')
fi
...
assert_equal "CTRL 0.5.1" "0.5.1" "$VCL_FLEET_VERSION"
VINCULA_NODE_VERSION=$(grep -E '^readonly VINCULA_VERSION=' "${PROJECT_DIR}/vincula.sh"|head -1|sed -E 's/.*=\"([^\"]+)\".*/\1/')
assert_equal "NODE 0.5.1" "0.5.1" "$VINCULA_NODE_VERSION"
...
# --- 0.4.5: shared operation journal (write/read/corrupt/retention/redaction) ---
assert_success "0.4.5 operation journal helpers" python3 - \
  "${PROJECT_DIR}/lib/vincula-fleet.py" "${TEST_TMP}/ws-opjournal" <<'PY'
import importlib.util, json, os, sys
from pathlib import Path

fleet_path, home = sys.argv[1], Path(sys.argv[2])
...
secret_detail = (
    "vless://11111111-1111-4111-8111-111111111111@h:443 "
    "age1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq "
    "ok-summary"
)
mod.append_operation_journal(
    operation="probe",
    target="lax",
    state="SUCCESS",
    exit_code=0,
...
rows = mod.read_operation_journal(limit=10)
assert len(rows) == 1, rows
r0 = rows[0]
assert r0["operation"] == "probe"
assert r0["target"] == "lax"
assert r0["controller_version"] == mod.VCL_FLEET_VERSION
assert "operation_id" in r0 and r0.get("started_at") and r0.get("finished_at")
assert r0.get("time")
detail = r0.get("detail") or ""
assert "vless://" not in detail
assert "11111111-1111-4111-8111-111111111111" not in detail
assert "age1qqqq" not in detail.lower()
assert "[redacted" in detail

# Corrupt line tolerance
path = mod.operation_journal_path(create=False)
with path.open("a", encoding="utf-8") as fh:
    fh.write("not-json\n")
    fh.write("{bad\n")
mod.append_operation_journal(
    operation="verify", state="FAIL", exit_code=1, target="lax"
)
rows2 = mod.read_operation_journal(limit=50)
assert len(rows2) == 2, rows2
assert {r["operation"] for r in rows2} == {"probe", "verify"}

# Retention: keep last OPERATION_JOURNAL_MAX_LINES
mod.OPERATION_JOURNAL_MAX_LINES = 5
for i in range(12):
    mod.append_operation_journal(
        operation=f"op{i}", state="SUCCESS", exit_code=0, target=str(i)
    )
text_lines = [
    ln for ln in path.read_text(encoding="utf-8").splitlines() if ln.strip()
]
# corrupt lines may remain; valid JSON rows bounded
valid = []
for ln in text_lines:
    try:
        valid.append(json.loads(ln))
    except json.JSONDecodeError:
        continue
assert len(valid) <= 5, len(valid)
assert valid[-1]["operation"] == "op11"
print("ok")
PY

# --- 0.4.5: legacy_seed.py + provision legacy argv contract ---
assert_success "0.4.5 legacy_seed + provision argv" python3 - \
  "${PROJECT_DIR}/lib/legacy_seed.py" \
  "${PROJECT_DIR}/lib/vincula-fleet.py" \
  "${TEST_TMP}/legacy-seed-unit" <<'PY'
import importlib.util
import os
import sys
from pathlib import Path

legacy_path, fleet_path, tmp = sys.argv[1], sys.argv[2], Path(sys.argv[3])
tmp.mkdir(parents=True, exist_ok=True)

ls_spec = importlib.util.spec_from_file_location("legacy_seed", legacy_path)
ls = importlib.util.module_from_spec(ls_spec)
sys.modules["legacy_seed"] = ls
...
def write_secret(path: Path, text: str, mode: int = 0o600) -> Path:
    path.write_text(text, encoding="utf-8")
    os.chmod(path, mode)
    return path


def good_uri(*, uuid: str = uuid_ok, pbk_val: str = pbk, host: str = server) -> str:
    return (
        f"vless://{uuid}@{host}:443"
        f"?encryption=none&flow=xtls-rprx-vision&security=reality"
        f"&sni=www.cloudflare.com&fp=chrome&pbk={pbk_val}&sid=abcd&type=tcp"
    )


uri_file = write_secret(tmp / "good.uri", good_uri() + "\n")
key_file = write_secret(tmp / "good.key", priv + "\n")

# good URI + matching key
seed = ls.load_legacy_seed(
    uri_file=uri_file,
...
assert seed.uuid == uuid_ok
assert seed.user_tag == "legacy-user"
assert seed.public_key == pbk

# mismatch pbk
bad_pbk_uri = write_secret(
    tmp / "bad-pbk.uri",
    good_uri(pbk_val="AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") + "\n",
)
try:
    ls.load_legacy_seed(
        uri_file=bad_pbk_uri,
        private_key_file=key_file,
        user_tag="legacy-user",
        advertised_server=server,
    )
    raise SystemExit("expected pbk mismatch")
except ls.LegacySeedError as exc:
    assert "does not match" in str(exc)
    assert priv not in str(exc)

# bad UUID
bad_uuid_uri = write_secret(tmp / "bad-uuid.uri", good_uri(uuid="not-a-uuid") + "\n")
try:
    ls.load_legacy_seed(
        uri_file=bad_uuid_uri,
        private_key_file=key_file,
        user_tag="legacy-user",
        advertised_server=server,
    )
    raise SystemExit("expected bad UUID")
except ls.LegacySeedError as exc:
    assert "UUID" in str(exc) or "uuid" in str(exc).lower()
...
try:
    ...
    raise SystemExit("expected owner tag refuse")
except ls.LegacySeedError as exc:
    assert "owner" in str(exc)

# insecure mode (group/world readable) refused — POSIX only
if hasattr(os, "getuid"):
    insecure = write_secret(tmp / "insecure.uri", good_uri() + "\n", mode=0o644)
    try:
        ls.load_legacy_seed(
            uri_file=insecure,
            private_key_file=key_file,
        ...
        raise SystemExit("expected insecure mode refuse")
    except ls.LegacySeedError as exc:
...
if hasattr(os, "symlink"):
    ...
    try:
        ...
        try:
            ...
            raise SystemExit("expected symlink refuse")
        except ls.LegacySeedError as exc:
            assert "symlink" in str(exc)
    except (OSError, NotImplementedError):
        pass  # Windows without privilege: skip

# provision argv: three flags required together; paths only
fleet_spec = importlib.util.spec_from_file_location("vincula_fleet", fleet_path)
...
# incomplete flags → CLI die (no install)
home = tmp / "fleet-home"
home.mkdir(parents=True, exist_ok=True)
os.environ["VCL_FLEET_HOME"] = str(home)
import subprocess
...
subprocess.check_call(
    [sys.executable, fleet_path, "init"],
    stdout=subprocess.DEVNULL,
    stderr=subprocess.DEVNULL,
)

def run_incomplete(extra: list[str]) -> tuple[int, str]:
    proc = subprocess.run(
        [
            sys.executable,
            fleet_path,
...
rc1, err1 = run_incomplete(["--legacy-vless-uri-file", str(uri_file)])
assert rc1 != 0, err1
assert "legacy seed requires" in err1
assert priv not in err1 and "vless://" not in err1

rc2, err2 = run_incomplete(
    [
...
assert rc2 != 0, err2
assert "legacy seed requires" in err2

# installer argv carries paths only (never URI/key body)
argv = prov.installer_remote_argv(
...
joined = " ".join(argv)
assert "--legacy-vless-uri-file" in argv
assert "--legacy-reality-private-key-file" in argv
assert "--legacy-user-tag" in argv
assert "/tmp/stage/legacy-user.uri" in argv
assert "/tmp/stage/legacy-reality.key" in argv
assert "legacy-user" in argv
assert "vless://" not in joined
assert priv not in joined
...
for action in root._subparsers._group_actions:
    if getattr(action, "choices", None) and "node" in action.choices:
        node_p = action.choices["node"]
        for a2 in node_p._subparsers._group_actions:
            if getattr(a2, "choices", None) and "provision" in a2.choices:
                prov_parser = a2.choices["provision"]
                break
assert prov_parser is not None, "provision subparser missing"
help_out = prov_parser.format_help()
assert "--legacy-vless-uri-file" in help_out
...
assert "instances" in node_detail
assert isinstance(node_detail["instances"], list)
assert "recent_usage" in node_detail
assert isinstance(node_detail["recent_usage"], list)
...
assert "Instance History" in static_app
assert "renderInstanceHistory" in static_app
assert "Top destinations (7d)" in static_app
assert "data.destinations" in static_app
assert "data.instances" in static_app
assert "data.recent_usage" in static_app
assert "controller_version" in static_app
static_index = (Path(static_dir) / "index.html").read_text(encoding="utf-8")
assert "Shared fleet operation journal" in static_index or "CLI + UI" in static_index
...
# 0.5.1 requires an explicit observe ref after migrating a pre-workspace
...
# schema: no forbidden + no imported_at as archive event field (P1-4)
...
arch_cols={r[1] for r in sqlite3.connect(str(arch)).execute("PRAGMA table_info(audit_events)")}
assert "imported_at" not in arch_cols, arch_cols
# equality ignores imported_at: same canonical fields, different imported_at → dedupe
only_imp=dict(row); only_imp["imported_at"]="2099-01-01T00:00:00Z"
imp_arch=tmp/"imp.vclaudit"
aa.create_archive(fleet_id="fleet-b6", created_at="2026-08-21T00:00:00Z", time_from="2026-08-01T00:00:00Z", time_to="2026-08-21T00:00:00Z",
  events=[only_imp], nodes=[], users=[], instances=[], dest=imp_arch)
r_imp=aa.restore_archive_into_cache(imp_arch, c, rebuild_daily_usage_for_node=m.rebuild_daily_usage_for_node)
assert r_imp["deduped"]==1 and r_imp["inserted"]==0 and r_imp["aggregated"]==0
r1=aa.restore_archive_into_cache(arch, c, rebuild_daily_usage_for_node=m.rebuild_daily_usage_for_node)
assert r1["deduped"]==1 and r1["inserted"]==0 and r1["aggregated"]==0
assert c.execute("SELECT last_export_seq FROM sync_cursor WHERE node_id=?",(nid,)).fetchone()[0]==cur0  # AC-4.1-05
# conflict: same key different payload → rollback; daily_usage not partially updated
du_before=c.execute("SELECT COUNT(*) FROM daily_usage WHERE node_id=?",(nid,)).fetchone()[0]
bad=tmp/"bad.vclaudit"; row2=dict(row); row2["upload_bytes"]=999
aa.create_archive(fleet_id="fleet-b6", created_at="2026-08-21T00:00:00Z", time_from="2026-08-01T00:00:00Z", time_to="2026-08-21T00:00:00Z",
  events=[row2], nodes=[], users=[], instances=[], dest=bad)
try:
  aa.restore_archive_into_cache(bad, c, rebuild_daily_usage_for_node=m.rebuild_daily_usage_for_node); raise AssertionError("expected ARCHIVE_CONFLICT")
...
assert c.execute("SELECT COUNT(*) FROM daily_usage WHERE node_id=?",(nid,)).fetchone()[0]==du_before
# fleet_id mismatch fail-closed (if known)
m.fleet_db_meta_set(c,"fleet_id","cache-fleet-id"); c.commit()
try:
  aa.restore_archive_into_cache(arch, c, rebuild_daily_usage_for_node=m.rebuild_daily_usage_for_node)
  raise AssertionError("expected ARCHIVE_FLEET_MISMATCH")
except SystemExit as e:
  assert "ARCHIVE_FLEET_MISMATCH" in str(e)
c.close()
PY
...
# archive must not persist imported_at
assert_success "B7 archive no imported_at" python3 - "$ARCH" <<'PY'
import sqlite3,sys
...
assert_equal "B7 restore dedupe" $'imported: 0\nduplicates_skipped: 2\naggregated: 0' "$(fleet audit archive restore "$ARCH")"
...
python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$TEST_TMP/b7-bad-fleet.vclaudit" "$B7_ALICE_UID" <<'PY'
import importlib.util,sys; from pathlib import Path
s=importlib.util.spec_from_file_location("vf",sys.argv[1]); m=importlib.util.module_from_spec(s); s.loader.exec_module(m)
aa=m.load_audit_archive_module(); nid="aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"; uid=sys.argv[3]
ev={"node_id":nid,"instance_id":"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb","event_id":1,"export_seq":1,"connection_id":"c1","generation":0,"user_id":uid,"user_tag":"alice","started_at":"2026-08-01T00:00:00Z","last_seen_at":"2026-08-01T01:00:00Z","closed_at":"2026-08-01T01:00:00Z","destination_host":"h","destination_ip":None,"destination_port":None,"network":None,"upload_bytes":10,"download_bytes":20}
aa.create_archive(fleet_id="00000000-0000-4000-8000-000000000099",created_at="2026-08-21T00:00:00Z",time_from="2026-08-01T00:00:00Z",time_to="2026-08-21T00:00:00Z",events=[ev],nodes=[],users=[],instances=[],dest=Path(sys.argv[2]))
PY
bad_fleet_err=$(fleet audit archive restore "$TEST_TMP/b7-bad-fleet.vclaudit" 2>&1) || bad_fleet_rc=$?
assert_equal "B7 fleet mismatch rc" "1" "${bad_fleet_rc:-0}"
assert_success "B7 fleet mismatch msg" bash -c 'grep -q ARCHIVE_FLEET_MISMATCH <<<"$0"' "$bad_fleet_err"
...
aa.create_archive(fleet_id=m.load_workspace_manifest()["fleet_id"],created_at="2026-08-21T00:00:00Z",time_from="2026-08-01T00:00:00Z",time_to="2026-08-21T00:00:00Z",events=[ev],nodes=[],users=[],instances=[],dest=Path(sys.argv[2]))
PY
du_before=$(_pycur "SELECT COUNT(*) FROM daily_usage")
...
assert_equal "B7 conflict daily_usage untouched" "$du_before" "$(_pycur "SELECT COUNT(*) FROM daily_usage")"
...
PY
assert_equal "B7 fresh imported" $'imported: 2\nduplicates_skipped: 0\naggregated: 1' "$(fleet audit archive restore "$ARCH")"
...
PY
# restore rebuilds daily_usage so stats top users shows restored traffic (P1-4)
export VCL_FLEET_STATS_NOW="2026-08-16"
stats_out=$(fleet stats top users --days 30 --json)
unset VCL_FLEET_STATS_NOW
assert_success "B7 stats after restore" python3 - "$stats_out" "$B7_ALICE_UID" <<'PY'
import json,sys
doc=json.loads(sys.argv[1]); uid=sys.argv[2]
assert doc["mode"]=="top_users"
rows=[r for r in doc["rows"] if r.get("user_id")==uid]
assert rows, doc
assert sum(int(r.get("bytes") or 0) for r in rows)==90  # (10+20)+(20+40)
PY
if command -v age >/dev/null 2>&1 || [[ -x $PROJECT_DIR/tests/fixtures/fake-age ]]; then
...
B3_PAYLOAD_SRC="${PROJECT_DIR}/dist/vincula-node-0.5.1.tar.gz"
...
src = Path(project) / "dist" / "vincula-node-0.5.1.tar.gz"
root = Path(tmp) / "b3-mismatch"
root.mkdir(parents=True, exist_ok=True)
tarball = root / "vincula-node-0.5.1.tar.gz"
...
(root / "vincula-node-0.5.1.tar.gz.sha256").write_text(
    f"{bad}  vincula-node-0.5.1.tar.gz\n", encoding="utf-8"
)
(root / "payload-manifest.json").write_text(
    json.dumps(
        {
            "controller_version": "0.4.2",
            "node_payload_version": "0.5.1",
            "sha256": actual,
...
actual = hashlib.sha256(tarball.read_bytes()).hexdigest()
(root / "vincula-node-0.5.1.tar.gz.sha256").write_text(
    f"{actual}  vincula-node-0.5.1.tar.gz\n", encoding="utf-8"
)
mani = {
    "controller_version": "0.4.2",
    "node_payload_version": "0.5.1",
...
patterns = (
    r"payload/vincula-node-0\.5\.1\.tar\.gz$",
    r"payload/vincula-node-0\.5\.1\.tar\.gz\.sha256$",
...
set -euo pipefail
cd "'"$PROJECT_DIR"'"
test -f dist/vincula-node-0.5.1.tar.gz
test -f dist/vincula-node-0.5.1.tar.gz.sha256
...
loaded = prov.verify_local_payload(resolved)
assert loaded["node_payload_version"] == "0.5.1"
...
B4_PAYLOAD_SRC="${PROJECT_DIR}/dist/vincula-node-0.5.1.tar.gz"
...
b4_stage_payload() {
  local root=$1
  mkdir -p "$root"
  cp -f "$B4_PAYLOAD_SRC" "$root/vincula-node-0.5.1.tar.gz"
  python3 - "$root" <<'PY'
import hashlib, json, sys
from pathlib import Path
root = Path(sys.argv[1])
tar = root / "vincula-node-0.5.1.tar.gz"
digest = hashlib.sha256(tar.read_bytes()).hexdigest()
(root / "vincula-node-0.5.1.tar.gz.sha256").write_text(
    f"{digest}  vincula-node-0.5.1.tar.gz\n", encoding="utf-8"
)
(root / "payload-manifest.json").write_text(
    json.dumps(
        {
...
assert_success "B4 remote digest mismatch refuses installer" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-digest-payload/vincula-node-0.5.1.tar.gz" \
...
assert_success "B4 provision success registers node" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-ok-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
...
assert_success "B4 REMOTE_READY_LOCAL_UNCOMMITTED on registry commit fail" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-commit-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, os, subprocess, sys
from pathlib import Path
...
assert_success "B4 success path calls sync --full once" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-sync-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, subprocess, sys
...
assert_success "P1-2 sudo install invokes sudo -n env VCL_SERVER bash vincula.sh" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-sudo-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
...
assert_success "P1-3 duplicate name blocked before remote install" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-dupname-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, io, os, subprocess, sys
from pathlib import Path
...
assert_success "P1-3 SystemExit on commit returns REMOTE_READY_LOCAL_UNCOMMITTED" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-sysexit-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, subprocess, sys
...
assert_success "P1-4 provision PARTIAL when initial sync fails" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/b4-partial-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, subprocess, sys
...
assert_success "P2-5 provision happy path cleans random staging dir" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/p2-5-ok-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, json, os, subprocess, sys
from pathlib import Path
...
assert_success "P2-5 digest failure cleans random staging dir" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/p2-5-digest-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
...
assert_success "P2-5 verify failure cleans random staging dir" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/p2-5-verify-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$B4_HK" <<'PY'
import importlib.util, io, json, os, subprocess, sys
from pathlib import Path
...
SEED_PAYLOAD_SRC="${PROJECT_DIR}/dist/vincula-node-0.5.1.tar.gz"
...
seed_stage_payload() {
  local root=$1
  mkdir -p "$root"
  cp -f "$SEED_PAYLOAD_SRC" "$root/vincula-node-0.5.1.tar.gz"
  python3 - "$root" <<'PY'
import hashlib, json, sys
from pathlib import Path
root = Path(sys.argv[1])
...
assert_success "0.4.5 seed E2E root harden + dual-user" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/seed-root-payload/vincula-node-0.5.1.tar.gz" \
...
assert_success "0.4.5 seed E2E sudo harden chown/chmod" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/seed-sudo-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$SEED_HK" \
    "$SEED_SECRETS/good.uri" "$SEED_SECRETS/good.key" <<'PY'
import importlib.util, os, subprocess, sys
from pathlib import Path
...
assert_success "0.4.5 seed E2E chown fail is zero-install" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/seed-chown-fail-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$SEED_HK" \
    "$SEED_SECRETS/good.uri" "$SEED_SECRETS/good.key" <<'PY'
import importlib.util, io, json, os, subprocess, sys
from pathlib import Path
...
assert_success "0.4.5 seed E2E chmod fail is zero-install" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/seed-chmod-fail-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$SEED_HK" \
    "$SEED_SECRETS/good.uri" "$SEED_SECRETS/good.key" <<'PY'
import importlib.util, io, os, subprocess, sys
from pathlib import Path
...
assert_success "0.4.5 seed E2E refuse existing VERSION clean host" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/seed-existing-payload/vincula-node-0.5.1.tar.gz" \
  python3 - "$PROJECT_DIR/lib/vincula-fleet.py" "$SEED_HK" \
    "$SEED_SECRETS/good.uri" "$SEED_SECRETS/good.key" <<'PY'
import importlib.util, io, os, subprocess, sys
from pathlib import Path
...
# Installer source guard + 0.3.x→0.5.1 migrate pin (identity preserve)
assert_success "0.5.1 vincula.sh refuses seed when VERSION exists" \
  grep -Fq 'Legacy seed requires a clean host.' "${PROJECT_DIR}/vincula.sh"
assert_success "0.5.1 is_supported_upgrade_from 0.3.1" \
  bash -c '
    set -euo pipefail
    # shellcheck disable=SC1091
    source "'"${PROJECT_DIR}"'/vincula.sh"
    [[ "$VINCULA_VERSION" == "0.5.1" ]]
    is_supported_upgrade_from 0.3.1
  '
assert_success "0.5.1 is_supported_upgrade_from 0.3.2" \
  bash -c '
    set -euo pipefail
    # shellcheck disable=SC1091
    source "'"${PROJECT_DIR}"'/vincula.sh"
  ...
  '
assert_success "0.5.1 migrate preserves UUID/Reality guards present" \
  bash -c '
...
# --- 0.5.1 Controller observation + upgrade (G1) ---
assert_success "0.5.1 build-controller packs observation modules" \
  grep -q 'lib/observation/capabilities.py' "${PROJECT_DIR}/scripts/build-controller.sh"
assert_success "0.5.1 build-controller packs node_upgrade" \
  grep -q 'lib/node_upgrade.py' "${PROJECT_DIR}/scripts/build-controller.sh"
assert_success "0.5.1 build-controller packs ssh_transport" \
  grep -q 'lib/ssh_transport.py' "${PROJECT_DIR}/scripts/build-controller.sh"

assert_success "0.5.1 schema_validate fixtures" python3 - \
...
assert_success "0.5.1 parse_stdout_json rejects Infinity/NaN" python3 - \
...
assert_success "0.5.1 observe/admin credential routing" python3 - \
  "${PROJECT_DIR}/lib/vincula-fleet.py" \
...
assert_success "obs050 capabilities OK when fake node 0.5.1" python3 - "$obs_cap_ok" <<'PY'
import json, sys
doc = json.loads(sys.argv[1])
...
assert_success "obs050 telemetry OK when fake node 0.5.1" python3 - "$obs_tel_ok" <<'PY'
import json, sys
doc = json.loads(sys.argv[1])
assert doc.get("state") == "OK", doc
...
assert doc.get("target_version") == "0.5.1", doc
assert doc.get("credential_class") == "observe", doc
PY
...
assert_success "obs050 upgrade apply SKIPPED when already 0.5.1" python3 - \
  "${PROJECT_DIR}/lib/vincula-fleet.py" <<'PY'
import importlib.util, os
from pathlib import Path
...
class FakeProv:
    ...
    def resolve_node_payload(self):
        return {"tar": "/dev/null", "sha256": "x", "version": "0.5.1"}
...
class FakeProv:
    ...
    def _remote_stage_paths(self, stage):
        return {
            ...
            "unpack": f"{stage}/vincula-node-0.5.1",
        }
...
def fake_obs(node, remote_cmd, **kwargs):
    ...
    if cmd[:2] == ["vcl", "identity"]:
        ident_n["n"] += 1
        ver = "0.3.1" if ident_n["n"] == 1 else "0.5.1"
        return (
            "OK",
            {
                "vincula_version": ver,
    ...
    if (
        ...
        assert cmd[1] == "/tmp/vcl-stage/vincula-node-0.5.1/bin/vincula", cmd
        return "OK", {"ok": True, "path": "/var/backups/vincula/upgrade-checkpoint-x"}, ""
...
(root / "payload-manifest.json").write_text(
    json.dumps({"controller_version": "0.5.1", "node_payload_version": "0.5.1", "sha256": digest,
                "supported_os": ["debian12"], "supported_arch": ["amd64"]}, indent=2) + "\n",
    encoding="utf-8",
)
PY
export VCL_NODE_ARCHIVE="$UPG_FAIL_PAYLOAD/vincula-node-0.5.1.tar.gz"
upg_fail_rc=0
...
mkdir -p "$UPG_PAYLOAD"
cp -f "${PROJECT_DIR}/dist/vincula-node-0.5.1.tar.gz" "$UPG_PAYLOAD/"
...
PY
export VCL_FLEET_HOME="$OBS050_HOME"
export VCL_NODE_ARCHIVE="$UPG_PAYLOAD/vincula-node-0.5.1.tar.gz"
export VCL_FAKE_UPGRADE=1
...
assert doc.get("to_version") == "0.5.1", doc
...
# --- G2 mixed-version fleet (0.3.x + 0.5.1) ---
...
ck2 = subprocess.run(
    [
        ...
        "--",
        "bash",
        "/tmp/stage/vincula-node-0.5.1/bin/vincula",
        "upgrade",
        "checkpoint",
        "--json",
    ],
...
B6_PAYLOAD_SRC="${PROJECT_DIR}/dist/vincula-node-0.5.1.tar.gz"
...
b6_stage_payload() {
  local root=$1
  mkdir -p "$root"
  cp -f "$B6_PAYLOAD_SRC" "$root/vincula-node-0.5.1.tar.gz"
  python3 - "$root" <<'PY'
import hashlib, json, sys
from pathlib import Path
root = Path(sys.argv[1])
...
export VCL_NODE_ARCHIVE="${TEST_TMP}/b6-good-payload/vincula-node-0.5.1.tar.gz"
...
export VCL_NODE_ARCHIVE="${TEST_TMP}/b6-dig-payload/vincula-node-0.5.1.tar.gz"
b6_dig_rc=0
...
assert_success "LIVE-P1 missing python3 provision succeeds" \
  env \
    ...
    VCL_NODE_ARCHIVE="${TEST_TMP}/live-p1-payload/vincula-node-0.5.1.tar.gz" \
...
export VCL_NODE_ARCHIVE="${TEST_TMP}/live-p1-payload/vincula-node-0.5.1.tar.gz"
json_rc=0
...
# §5 fingerprint of real HOME known_hosts (also re-checked in finish/finish_fleet).
_real_home="${FLEET_SAVED_HOME:-${SAVED_SUITE_HOME:-}}"
_real_fp="${FLEET_REAL_KH_FP:-${SUITE_REAL_KH_FP:-}}"
if [[ -n "${_real_fp}" && -n "${_real_home}" && -f "${_real_home}/.ssh/known_hosts" ]]; then
  _after=$(sha256sum -- "${_real_home}/.ssh/known_hosts" 2>/dev/null | awk '{print $1}')
  if [[ "${_after}" == "${_real_fp}" ]]; then
    pass "P1 §5: real HOME known_hosts unchanged at fleet suite end"
  else
    fail "P1 §5: real HOME known_hosts changed at fleet suite end"
  fi
elif [[ -n "${_real_home}" && -f "${_real_home}/.ssh/known_hosts" && -z "${_real_fp}" ]]; then
  fail "P1 §5: known_hosts appeared under real HOME during fleet suite"
else
