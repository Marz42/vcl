...
This is the **living-tree gate** for `0.3.1-rc1`. It is **not** a continuation or rewrite of the 0.3.0 freeze record. Frozen-tag evidence stays in [`legacy/release-readiness-0.3.0.md`](legacy/release-readiness-0.3.0.md) / [`legacy/known-issues-0.3.0.md`](legacy/known-issues-0.3.0.md) (read-only). Older gates: [`legacy/`](legacy/).
...
| **P1-04** | Installer allowlist includes `0.3.0` and `0.3.1-dev` (excludes `0.3.0-dev` / `0.3.1-rc1`). Fixture keep tests cover identity, credentials, Reality, accounting, dual-service. | **CLOSED** (fixture). Live upgrade still open (below). |
...
B14 **PASS** (2026-08-18). B15 Local Audit UI is on the tree. Recommendation stays **NOT READY** until live `0.3.0 → 0.3.1-rc1` upgrade is closed or waived.
