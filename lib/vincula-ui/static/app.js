...
(function () {
  ...
  function statePill(v) {
    ...
    return `<span class="state ${s}">${escapeHtml(s)}</span>`;
  }
  ...
  function renderWarnings(list) {
    ...
    bar.innerHTML = items
      ...
      .map(
        (w) =>
          `<span class="chip ${w.level || "amber"}">${escapeHtml(w.message || w.code)}</span>`
      )
      .join("");
  }
...
source: ${escapeHtml(u.source || "—")}</pre>
  ...
  async function doSync() {
    const reseed = window.confirm(
      "Sync fleet audit cache now?\n\nOK = sync\nCancel = abort\n\nAfter OK you will be asked about --reseed."
    );
    if (!reseed) return;
    let reseedName = null;
    if (
      window.confirm(
        "Reseed a node? (wipes local audit for that node then pulls --after 0)\n\nCancel = normal sync\nOK = enter node name next"
      )
    ) {
      reseedName = window.prompt("Node name to --reseed (required):", "");
      if (!reseedName) {
        toast("Sync cancelled (empty reseed name)");
        return;
      }
    }
    try {
      toast("Syncing…");
      const body = reseedName ? { reseed: reseedName.trim() } : {};
      const data = await postJson("/api/sync", body);
