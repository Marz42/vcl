...
assert_equal "runs when read from standard input" "vincula 0.2.7-dev" \
...
assert_failure "does not migrate the current version" is_supported_upgrade_from 0.2.7-dev
...
assert_success "keeps management state private by design" grep -q '^project_version = "0.2.7-dev"$' "${TEST_TMP}/config.toml"
...
assert_success "accountd unit version stamp is 0.2.7-dev" \
  grep -q 'Vincula-Version: 0.2.7-dev' "${PROJECT_DIR}/lib/vincula-accountd.service"
