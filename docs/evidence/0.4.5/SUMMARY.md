...
| **4.5-05** | **PASS (offline) / PENDING LIVE** | Fail-close wrong key/URI/mode/symlink/incomplete flags; sudo harden rc; unit + E2E in `tests/test-fleet.sh` |
...
| **4.5-08** | **PASS (offline)** | Closes 0.4.4 PARTIAL (AC-4.3-05): Node Instance History + recent usage; User destinations = host/bytes/connections (no network); shared CLI+UI operation journal (replace backup/restore sub-ops + `audit_archive_restore`) |
| **4.5-09** | **PASS (offline) / PENDING LIVE+review** | Offline tests **1758** passed; digests OK; docs updated; LIVE + human review remain |
...
## Offline gate (review-fix)

- Offline tests: **1758** passed (`bash tests/test.sh`, includes seed E2E / tag-URI rejects / journal lock)
- `git diff --check` on first-party code paths after review-fix: clean
- Artifact SHAs below are a **retained record of one build**, not a reproducible proof of source→byte identity (zip/tar timestamps and packaging can differ across builds):
  - `dist/vincula-node-0.3.2.tar.gz` — `f0324052e2c98c99d8e8c5a4d2a24f542e929d5d07be897363e5b0e91524ce22`
  - `dist/vincula-controller-0.4.5.zip` — `a8f86c62898feb2bc222e79282dc21cdaf13600ad75fe6652edeaec8798d7e6d`
