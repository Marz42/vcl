...
- **B15 / Local Audit UI:** `vcl-fleet ui [--host 127.0.0.1] [--port 8765]` serves a loopback-only read-only UI (Overview / Audit / Health) from `lib/vincula-ui/` (stdlib `ThreadingHTTPServer` + static HTML/CSS/JS). Non-loopback binds refuse. Data from `$FLEET_HOME`; Refresh/Verify/Sync reuse controller SSH paths; identity mutations stay CLI (recipes panel copy-only). No VLESS URI / Reality keys / Clash secret on default pages. Controller zip packs UI members into `controller.lock`. AC-3.1 fixture coverage in `tests/test-fleet.sh`.
